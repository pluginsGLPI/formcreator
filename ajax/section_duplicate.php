...
if (!$section->canCreateItem()) {
    http_response_code(403);
    echo __('You don\'t have right for this action', 'formcreator');
    exit;
}

if (! $newSectionId = $section->duplicate(['id' => $sectionId])) {
    http_response_code(500);
    echo __('Could not duplicate the section', 'formcreator');
    exit;
}
$section = new PluginFormcreatorSection();
$section->getFromDB($newSectionId);
