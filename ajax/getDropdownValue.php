...
if (isset($_POST["entity_restrict"]) && !is_array($_POST["entity_restrict"]) && (substr($_POST["entity_restrict"], 0, 1) === '[') && (substr($_POST["entity_restrict"], -1) === ']')) {
...
if ($item instanceof CommonTreeDropdown) {
    ...
    if ($item->isEntityAssign() && ($one_item < 0)) {
        ...
        if (isset($_POST["entity_restrict"]) && !($_POST["entity_restrict"] < 0)) {
            $where .= getEntitiesRestrictRequest("AND", $table, "entities_id", $_POST["entity_restrict"], $multi);
...
//       $outputval = Dropdown::getDropdownName($table, $_POST['value']);
        ...
        if ($DB->numrows($result)) {
            ...
            while ($data = $DB->fetch_assoc($result)) {
                ...
                $i = 0;
                foreach ($datastoadd as $array_data) {
                    if ($array_data['text'] == $outputval) {
                        $exist = true;
                    }
                    $i++;
                }
...
if (($one_item >= 0) && isset($datas[0])) {
    ...
    $_SESSION['Object_applications'] = $datas;
    $ret['results'] = $datas;
    $ret['count'] = $count;
