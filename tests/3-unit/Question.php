...
class Question extends CommonTestCase {
   ...
   public function providerPrepareInputForAdd() {
      ...
      $section2 = $this->getSection(
         [],
         [
            'access_rights' => Form::ACCESS_PUBLIC,
         ]
      );
      $dataset = [
         [
   ...
   public function testImport() {
      ...
      $testedClass = $this->getTestedClassName();
      $linker = new Linker();
      $questionId = $testedClass::import($linker, $input, $section->getID());
      $this->integer($questionId)->isGreaterThan(0);

      unset($input['uuid']);

      $this->exception(
         function() use($linker, $input, $section, $testedClass) {
            $testedClass::import($linker, $input, $section->getID());
         }
      )->isInstanceOf(\GlpiPlugin\Formcreator\Exception\ImportFailureException::class)
      ->hasMessage('UUID or ID is mandatory for Question'); // passes

      $input['id'] = $questionId;
      $questionId2 = $testedClass::import($linker, $input, $section->getID());
   ...
   public function providerGetTranslatableStrings() {
      ...
      //$expectedCount = count(\Fields::getTypes());
   ...
   public function testCountItemsToImport() {
      ...
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::countItemsToImport($input);
   ...
   public function testGetQuestionsFromForm() {
      ...
      $testedClass = $this->getTestedClassName();
      $questionsGenerator = $testedClass::getQuestionsFromForm($form->getID());
      $questionIds = [];
      $count = 0;
   ...
   public function testGetQuestionsFromSection() {
      ...
      $testedClass = $this->getTestedClassName();
      $questionsGenerator = $testedClass::getQuestionsFromSection($section->getID());
      $questionIds = [];
      foreach ($questionsGenerator as $itemId => $item) {
         $questionIds[] = $itemId;
      }
