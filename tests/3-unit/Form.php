...
use GlpiPlugin\Formcreator\Form_Profile;
...
class Form extends CommonTestCase {
   ...
   public function testGetEnumAccessType() {
      ...
      $output = $testedClass::getEnumAccessType();
      $this->array($output)->isEqualTo([
         $testedClass::ACCESS_PUBLIC     => __('Public access', 'formcreator'),
         $testedClass::ACCESS_PRIVATE    => __('Private access', 'formcreator'),
         $testedClass::ACCESS_RESTRICTED => __('Restricted access', 'formcreator'),
      ]);
   }
   ...
   public function testCanPurgeItem() {
      ...
      $formAnswer->add([
         $testedClass::getForeignKeyField() => $form->getID(),
      ]);
      $this->restoreDebug();
   ...
   public function testDefineTabs() {
      ...
      $expected = [
         'GlpiPlugin\Formcreator\Form$main' => 'Form',
         'GlpiPlugin\Formcreator\Form_Validator$1' => 'Validators',
         'GlpiPlugin\Formcreator\Question$1' => 'Questions',
         'GlpiPlugin\Formcreator\FormAccessType$1' => 'Access types',
         'GlpiPlugin\Formcreator\Form$1' => 'Targets',
         'GlpiPlugin\Formcreator\Form$2' => 'Preview',
         'GlpiPlugin\Formcreator\Form$3' => 'Form answer properties',
         'GlpiPlugin\Formcreator\FormAnswer$1' => 'Form answers',
         'GlpiPlugin\Formcreator\Form_Language$1' => 'Form languages',
         'Log$1' => 'Historical',
      ];
      $this->array($output)
         ->isEqualTo($expected)
   ...
   public function testUpdateValidators() {
      ...
      $form = $this->getForm([
         'validation_required' => Form_Validator::VALIDATION_GROUP,
   ...
   public function providerIsPublicAcess() {
      ...
      return [
         'not public' => [
            'input' => [
               'access_rights' => (string)  $testedClass::ACCESS_PRIVATE,
         ...
         'public' => [
            'input' => [
               'access_rights' => (string)  $testedClass::ACCESS_PUBLIC,
               'name' => $this->getUniqueString()
            ],
            'expected' => true,
         ],
         'by profile' => [
            'input' => [
               'access_rights' => (string)  $testedClass::ACCESS_RESTRICTED,
               'name' => $this->getUniqueString()
            ],
            'expected' => false,
         ],
   ...
   public function providerGetByItem() {
      ...
      $dataset = [
         [
            'item'         => $section,
            'expectedType' => $testedClass::getType(),
            'expected'     => true,
         ],
         [
            'item'         => new Section(),
   ...
   public function testGetByItem($item, $expectedType, $expected) {
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::getByItem($item);
      if ($expected === false) {
   ...
   public function testImport() {
      ...
      $input = [
         ...
         '_entity' => 'Root entity',
         'is_recursive' => '0',
         'access_rights' =>  $testedClass::ACCESS_RESTRICTED,
         'description' => '',
         'content' => '',
         '_plugin_formcreator_category' => '',
         'is_active' => '1',
      ...
      $linker = new Linker ();
      $formId =  $testedClass::import($linker, $input);
      $this->integer($formId)->isGreaterThan(0);

      unset($input['uuid']);

      $this->exception(
         function() use($linker, $input, $testedClass) {
             $testedClass::import($linker, $input);
         }
      )->isInstanceOf(\GlpiPlugin\Formcreator\Exception\ImportFailureException::class)
      ->hasMessage('UUID or ID is mandatory for Form'); // passes

      $input['id'] = $formId;
      $formId2 =  $testedClass::import($linker, $input);
   ...
   public function testDuplicate() {
      ...
      // check sections
      $all_sections = Section::getSectionsFromForm($form->getID());
      $all_new_sections = Section::getSectionsFromForm($new_form->getID());
      ...
      $all_targetTickets = (new TargetTicket())->getTargetsForForm($form->getID());
      $all_new_targetTickets = (new TargetTicket())->getTargetsForForm($new_form->getID());
      ...
      $all_targetChanges = (new TargetChange())->getTargetsForForm($form->getID());
      $all_new_targetChanges = (new TargetChange())->getTargetsForForm($new_form->getID());
   ...
   public function providerGetBestLanguage() {
      $testedClass = $this->getTestedClassName();
      $formFk =  $testedClass::getForeignKeyField();
    ...
    * @return void
    */
   public function testGetBestLanguage($form, string $sessionLanguage, string $expected) {
   ...
   public function testGetTranslationDomain() {
      ...
      $output = $testedClass::getTranslationDomain($instance->getID(), 'en_US');
      $this->string($output)->isEqualTo('form_' . $instance->getID() . '_en_US' );

      $output = $testedClass::getTranslationDomain($instance->getID(), 'it_IT');
   ...
   public function testGetTranslationFile() {
      ...
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::getTranslationFile($instance->getID(), 'en_US');
      $this->string($output)->isEqualTo(GLPI_LOCAL_I18N_DIR . '/formcreator/form_' . $instance->getID() . '_en_US.php' );

      $output = $testedClass::getTranslationFile($instance->getID(), 'fr_CA');
   ...
   public function testGetTranslatableStrings() {
      ...
      foreach ($data['forms'] as $formData) {
         ...
         $formId = $form->import(new Linker(), $formData);
   ...
   public function testCheckImportVersion($version, $expected) {
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::checkImportVersion($version);
      $this->boolean($output)->isEqualTo($expected);
   }
   ...
   public function testAdd() {
      ...
      $this->string($row['name'])->isEqualTo(Section::getTypeName(1));
   }
   ...
   public function testCountAvailableForm() {
      ...
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::countAvailableForm();

      // Debug information in case test fails
      if ($output != 2) {
         $listQuery = $testedClass::getFormListQuery();
         $listQuery['SELECT'] = $testedClass::getTable() . '.name';
