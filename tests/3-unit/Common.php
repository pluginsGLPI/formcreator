...
class Common extends CommonTestCase {
   ...
   public function testGetFormcreatorRequestTypeId() {
      ...
      $requestTypeId = $testedClass::getFormcreatorRequestTypeId();
   ...
   public function testIsNotificationEnabled() {
      ...
      $output = $testedClass::isNotificationEnabled();
   ...
   public function testSetNotification() {
      ...
      $CFG_GLPI['use_notifications'] = '1';
      $testedClass::setNotification(false);
      $this->integer((int) $CFG_GLPI['use_notifications'])->isEqualTo('0');

      $testedClass::setNotification(true);
   ...
   public function testGetPictoFilename($version, $expected) {
      ...
      $output = $testedClass::getPictoFilename($version);
   ...
   public function testPrepareBooleanKeywords($input, $expected) {
      ...
      $output = $testedClass::prepareBooleanKeywords($input);
      $this->string($output)->isEqualTo($expected);
   }
   ...
   public function testGetCaptcha() {
      ...
      $output = $testedClass::getCaptcha($captchaId);
   ...
   public function testCheckCaptcha() {
      ...
      $output = $testedClass::checkCaptcha($captchaId, $challenge);
      $this->boolean($output)->isTrue();
      $output = $testedClass::checkCaptcha($captchaId, $challenge . 'foo');
      $this->boolean($output)->isFalse();
   }
   ...
   public function testCleanOldCaptchas() {
      ...
      $output = $testedClass::getCaptcha('captcha1');
      $challenge = $_SESSION['plugin_formcreator']['captcha']['captcha1']['phrase'];
      sleep(2); // Wait 5 seconds
      $output = $testedClass::getCaptcha('captcha2');
      $output = $testedClass::checkCaptcha('captcha1', $challenge, 1);
      $this->boolean($output)->isFalse();
   ...
   public function providerGetTicketStatusForIssue() {
      ...
      // Common::getTicketStatusForIssue (total 18 test cases)
      $expectedStatus = [
            Ticket::INCOMING,
            Ticket::ASSIGNED,
            Ticket::PLANNED,
      ...
      // Common::getTicketStatusForIssue (total 4 test cases)
      $expectedStatus = [
         Ticket::INCOMING,
         Ticket::ASSIGNED,
         Ticket::PLANNED,
      ...
      foreach ($expectedStatus as $ticketStatus) {
         ...
         $dataSet = [
            'ticket' => $ticket,
            'expected' => FormAnswer::STATUS_WAITING
         ];
         yield $dataSet;
      }
      ...
      foreach ($expectedStatus as $ticketStatus) {
         ...
         $dataSet = [
            'ticket' => $ticket,
            'expected' => FormAnswer::STATUS_REFUSED
         ];
         yield $dataSet;
      }
   ...
   public function testGetTicketStatusForIssue($ticket, $expected) {
      ...
      $output = $testedClass::getTicketStatusForIssue($ticket);
      $this->integer((int) $output)->isEqualTo($expected);
   }

   public function testGetFormAnswer() {
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::getFormAnswer();
      $this->string($output->getType())->isEqualTo(FormAnswer::class);
   }

   public function testGetFormAnswerItemtype() {
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::getFormAnswerItemtype();
      $this->string($output)->isEqualTo(FormAnswer::class);
   }
   ...
   public function testGetInterface() {
      ...
      // test Public access
      Session::destroy();
      $output = $testedClass::getInterface();
      ...
      $entityConfig->update([
         'id' => $entityConfig->getId(),
         'replace_helpdesk' => EntityConfig::CONFIG_SIMPLIFIED_SERVICE_CATALOG,
      ]);
      $this->login('post-only', 'postonly');
      $output = $testedClass::getInterface();
      ...
      $entityConfig->update([
         'id' => $entityConfig->getId(),
         'replace_helpdesk' => EntityConfig::CONFIG_EXTENDED_SERVICE_CATALOG,
      ]);
      $this->login('post-only', 'postonly');
      $output = $testedClass::getInterface();
      $this->string($output)->isEqualTo('servicecatalog');
   ...
   public function providerHookRedefineMenu() {
      ...
      EntityConfig::getUsedConfig('replace_helpdesk', $entityId);
   ...
   public function testHookRedefineMenu($input, $expected) {
      $testedClass = $this->getTestedClassName();
      $output = $testedClass::hookRedefineMenu($input);
