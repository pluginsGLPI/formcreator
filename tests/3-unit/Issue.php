...
class Issue extends CommonTestCase {
   ...
   public function providerGetSyncIssuesRequest_formAnswerWithSeveralTickets() {
      ...
      $this->boolean($targetTicket1->isNewItem())->isFalse();
      $targetTicket2 = new TargetTicket();
      $targetTicket2->add([
         'plugin_formcreator_forms_id' => $form->getID(),
         'name' => 'bar',
      ]);
   ...
   public function testGetSyncIssuesRequest($item, $expected) {
      ...
      if ($item->getType() == Ticket::class) {
         ...
         if (count($unwantedItems) > 0) {
            ...
            // WHERE itemtype = 'FormAnswer' AND items_id IN ( <list of numbers> )
            $result = $DB->request([
               'FROM'  => $request,
               'WHERE' => $unwantedWhere,
            ]);
      ...
      if ($item->getType() == FormAnswer::class) {
         $unwantedItems = $DB->request([
            'SELECT' => ['tickets_id'],
            'FROM' => \Item_Ticket::getTable(),
            'WHERE' => [
