...
use GlpiPlugin\Formcreator\Target\TargetInterface;
use Group;
use Html;
use KnowbaseItem;
...
class Form extends CommonDBTM implements
ExportableInterface,
...
TranslatableInterface
{
   ...
   public static function getEnumShowRule() : array {
      return Condition::getEnumShowRule();
   }
   ...
   public function canPurgeItem() {
      ...
      if ($DbUtil->countElementsInTable(FormAnswer::getTable(), $criteria) > 0) {
         return false;
      }
   ...
   public static function getMenuContent() {
      ...
      $requests_image   = '<i class="fa fa-paper-plane"
                                 title="' . Issue::getTypeName(Session::getPluralNumber()) . '"></i>';

      $menu['links']['search']          = FormList::getSearchURL(false);
      $menu['links'][$validation_image] = FormAnswer::getSearchURL(false).'?criteria[0][link]=AND&criteria[0][field]=8&criteria[0][searchtype]=equals&criteria[0][value]=' . FormAnswer::STATUS_WAITING;
      $menu['links'][$import_image]     = Form::getFormURL(false)."?import_form=1";
      $menu['links'][$requests_image]   = Issue::getSearchURL(false);
      return $menu;
   }
   ...
   public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
      if ($item instanceof Form) {
         return [
            ...
            2 => __('Preview'),
            3 => FormAnswer::getTypeName(1) . ' ' .__('properties', 'formcreator'),
         ];
      }
   ...
   public function defineTabs($options = []) {
      ...
      $this->addStandardTab(Form_Validator::class, $ong, $options);
      $this->addStandardTab(Question::class, $ong, $options);
      $this->addStandardTab(FormAccessType::class, $ong, $options);
      $this->addStandardTab(self::class, $ong, $options);
      $this->addStandardTab(FormAnswer::class, $ong, $options);
      $this->addStandardTab(Form_Language::class, $ong, $options);
   ...
   public static function getFormList(int $rootCategory = 0, string $keywords = '', bool $helpdeskHome = false): array {
      ...
      $display_format = EntityConfig::getUsedConfig('tile_design', Session::getActiveEntity());

      $table_cat      = getTableForItemType(Category::class);
      $table_form     = getTableForItemType(Form::class);
      $table_section  = getTableForItemType(Sections::class);
      $table_question = getTableForItemType(Questions::class);

      $categoryFk = Category::getForeignKeyField();
      ...
      $formList['LEFT JOIN'][$table_section] = [
         'FKEY' => [
            $table_section => Form::getForeignKeyField(),
            $table_form => 'id',
         ]
      ];
      $formList['LEFT JOIN'][$table_question] = [
         'FKEY' => [
            $table_question => Section::getForeignKeyField(),
            $table_section => 'id'
         ]
      ];
      if ($helpdeskHome) {
      ...
      if ($rootCategory != 0) {
         ...
         $onlyDefault = EntityConfig::getUsedConfig('default_form_list_mode', Session::getActiveEntity());
         if ($onlyDefault == EntityConfig::CONFIG_DEFAULT_FORM_LIST_DEFAULT) {
      ...
      if (!empty($keywords)) {
         $keywordsWithWilcards = $DB->escape(Common::prepareBooleanKeywords($keywords));
         $formList['WHERE']['AND'][] = [
            'OR' => [
      ...
      if (EntityConfig::getUsedConfig('is_kb_separated', Session::getActiveEntity()) != EntityConfig::CONFIG_KB_DISTINCT
         ...
         foreach ($result_faqs as $faq) {
            $formList[] = [
               ...
               'is_default'       => false,
               'tile_template'    => EntityConfig::CONFIG_UI_FORM_MASONRY
            ];
         }
      }
   ...
   protected function showHeader(): void {
      $header = EntityConfig::getUsedConfig('is_header_visible', Session::getActiveEntity(), 'header');
   ...
   protected function showMyLastForms() : void {
      ...
      $search = Search::getDatas(FormAnswer::class, $criteria, $showColumns);
         ...
         foreach ($search['data']['rows'] as $formAnswer) {
            ...
            switch ($formAnswer['raw']["{$rawKeyBase}_8"]) {
               ...
               case FormAnswer::STATUS_REFUSED:
                  ...
                  break;
               case FormAnswer::STATUS_ACCEPTED:
   ...
   public function displayUserForm() : void {
      ...
      TemplateRenderer::getInstance()->display('@formcreator/pages/userform.html.twig', [
         ...
         'options' => [
            ...
            'use_captcha' => ($this->fields['access_rights'] == Form::ACCESS_PUBLIC
   ...
   public function prepareInputForAdd($input) {
      ...
      if (!$this->skipChecks) {
         if (!$this->checkConditionSettings($input)) {
            $input['show_rule'] = Condition::SHOW_RULE_ALWAYS;
         }
   ...
   public function post_updateItem($history = 1) {
      ...
      $this->update1NTableData(Form_User::class, 'users');
      $this->update1NTableData(Form_Group::class, 'groups');
      $this->update1NTableData(Form_Profile::class, 'profiles');
   }
   ...
   private function updateValidators() : void {
      ...
      if ($this->input['validation_required'] == Form_Validator::VALIDATION_NONE) {
         return;
      }
      if ($this->input['validation_required'] == Form_Validator::VALIDATION_USER
         && empty($this->input['_validator_users'])) {
         return;
      }
      if ($this->input['validation_required'] == Form_Validator::VALIDATION_GROUP
      ...
      switch ($this->input['validation_required']) {
         case Form_Validator::VALIDATION_USER:
            $validators = $this->input['_validator_users'];
            $validatorItemtype = User::class;
            break;
         case Form_Validator::VALIDATION_GROUP:
   ...
   /**
    * Execute massive action for From
    *
    * @since version 0.85
    *
   ...
   public static function processMassiveActionsForOneItemtype(MassiveAction $ma, CommonDBTM $item, array $ids) {
      switch ($ma->getAction()) {
         case 'Duplicate' :
            if (!Session::haveRight(Form::$rightname, CREATE)) {
   ...
   public static function countAvailableForm(): int {
      ...
      $formProfileTable = Form_Profile::getTable();
      $formUserTable = Form_User::getTable();
      $formGroupTable = Form_Group::getTable();
   ...
   public function export(bool $remove_uuid = false): array {
      ...
      /** @var Category */
      $formCategory = Category::getById($export['plugin_formcreator_categories_id']);
      ...
      $subItems = [
         ...
         '_users'        => Form_User::class,
         '_groups'       => Form_Group::class,
         '_sections'     => Section::class,
         '_conditions'   => Condition::class,
         '_targets'      => self::getTargetTypes(),
         '_validators'   => Form_Validator::class,
         '_translations' => Form_Language::class,
      ];
   ...
   public static function import(Linker $linker, $input = [], $containerId = 0) {
      ...
      $formCategoryFk = Category::getForeignKeyField();
      $formCategoryId = 0;
   ...
   public function showForCentral() {
      ...
      $result = Category::getAvailableCategories();
      ...
      $form_query = Form::getFormListQuery();
      $form_query['SELECT'] = [
      ...
      foreach ($result_forms as $row) {
         if ($currentCategoryId != $row[$formCategoryFk]) {
            ...
            echo '<tr class="noHover" data-itemtype="' . str_replace('\\', '_', Category::getType()) .  '" data-id="' . $currentCategoryId . '"><th>' . $categories[$currentCategoryId] . '</th></tr>';
         }

         // Show a row for the form
         $language = $_SESSION['glpilanguage'];
         $domain = Form::getTranslationDomain($row['id'], $language);
         $phpfile = self::getTranslationFile($row['id'], $_SESSION['glpilanguage']);
         if (file_exists($phpfile)) {
            $TRANSLATE->addTranslationFile('phparray', $phpfile, $domain, $_SESSION['glpilanguage']);
         }

         echo '<tr class="tab_bg_' . ($i % 2 +1) . '" data-itemtype="' . str_replace('\\', '_', Form::getType()) . '" data-id="' . $row['id'] . '">';
         echo '<td>';
   ...
   public static function getByItem(?CommonDBTM $item): ?self {
      ...
      if ($item instanceof TargetInterface) {
         $form->getFromDB($item->fields[$formFk]);
      }
   ...
   public function getFields() : array {
      ...
      $questionsGenerator = Question::getQuestionsFromForm($this->getID());
   ...
   public function post_getFromDB() {
      ...
      if ($language != $this->fields['language']) {
         ...
         $domain = Form::getTranslationDomain($language, $formId);
         $TRANSLATE->getEventManager()->attach(
            Translator::EVENT_MISSING_TRANSLATION,
            static function (EventInterface $event) use ($formId, $domain, $TRANSLATE) {
               if ($event->getParams()['text_domain'] == $domain) {
                  $file = Form::getTranslationFile($formId);
                  if (!is_readable($file)) {
                     return;
                  }
      ...
      $this->load1NTableData(Form_User::class, 'users');
      $this->load1NTableData(Form_Group::class, 'groups');
      $this->load1NTableData(Form_Profile::class, 'profiles');
   }
   ...
   public function getTranslatableStrings(array $options = []) : array {
      ...
      foreach (Section::getSectionsFromForm($this->getID()) as $section) {
   ...
   public function getBestLanguage() {
      ...
      $formLanguageTable = Form_Language::getTable();
      ...
      $result = $DB->request([
         ...
         'LEFT JOIN' => [
            $formTable => [
               'FKEY' => [
                  $formTable => 'id',
                  $formLanguageTable => Form::getForeignKeyField(),
               ],
            ],
         ],
         'WHERE'     => [
   ...
   public function canViewForRequest(): bool {
      ...
      if ($this->fields['access_rights'] == self::ACCESS_RESTRICTED
         && !FormAccessType::canSeeRestrictedForm($this)
      ) {
         return false;
      }
   ...
   public static function getFormListQuery() {
      ...
      $dbUtils           = new DbUtils();
      $formTable         = getTableForItemType(Form::class);
      $formLanguageTable = getTableForItemType(Form_Language::class);

      $entityRestrict = $dbUtils->getEntitiesRestrictCriteria($formTable, '', '', true, false, (new Form())->maybeRecursive());
      $condition = [
         'FROM' => $formTable,
         'LEFT JOIN' => [
            $formLanguageTable => [
         ...
         'WHERE' => [
            'AND' => [
               ...
               [
                  'OR' => [
                     'access_rights' => ['!=', Form::ACCESS_RESTRICTED],
                     FormAccessType::getRestrictedFormListCriteria(),
                  ],
               ],
