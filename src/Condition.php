...
class Condition extends CommonDBChild implements ExportableInterface
{
   use ExportableTrait;
   ...
   public static function import(Linker $linker, array $input = [], int $containerId = 0) {
      ...
      $linked = $linker->getObject($input['plugin_formcreator_questions_id'], Question::class);
      if ($linked === false) {
         $linked = new Question();
   ...
   public static function countForItem(ConditionnableInterface $item): int {
      /** @var CommonDBTM $item */
      return (new DbUtils())->countElementsInTable(Condition::getTable(), [
         'itemtype' => $item::getType(),
         'items_id' => $item->getID(),
      ]);
   }
    ...
    *
    * @param ConditionnableInterface $item item where conditions applies to
    * @return void
    */
   public function showConditionsForItem(ConditionnableInterface $item) {
    ...
    *
    * @param ConditionnableInterface $item
    * @return void
    */
   public static function getQuestionsExclusion(?ConditionnableInterface $item) {
      if ($item === null) {
         return [];
      }
      ...
      if ($item instanceof Section) {
         if ($item->isNewItem()) {
            ...
            if (count($sectionsList) > 0) {
               $questionListExclusion[] = [
                  Section::getForeignKeyField() => $sectionsList,
               ];
            }
   ...
   public function getConditionHtml(CommonDBTM $parent): string {
      ...
      if (!is_subclass_of($itemtype, ConditionnableInterface::class)) {
         // security check
         throw new \RuntimeException("$itemtype is not a " . ConditionnableInterface::class);
      }
