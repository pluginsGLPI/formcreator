...
trait ConditionnableTrait
{
   ...
   public function checkConditions(array $input): bool {
      ...
      while (count($input['show_condition']) > 0) {
         ...
         if ($showCondition == Condition::SHOW_CONDITION_REGEX) {
            $regex = Toolbox::stripslashes_deep($value);
            $success = Common::checkRegex($regex);
            if (!$success) {
   ...
   public function checkConditionSettings(array $input): bool {
      ...
      if ($showRule == Condition::SHOW_RULE_ALWAYS) {
         return false;
      }
   ...
   public function updateConditions($input) : bool {
      ...
      if (!isset($this->input['show_rule']) && isset($this->fields['show_rule']) && $this->fields['show_rule'] == Condition::SHOW_RULE_ALWAYS) {
      ...
      if (isset($this->input['show_rule']) && $this->input['show_rule'] == Condition::SHOW_RULE_ALWAYS) {
