...
class Issue extends CommonDBTM {
   ...
   public static function getSyncIssuesRequest() : AbstractQuery {
      ...
      $formAnswerTable = FormAnswer::getTable();
      ...
      // must match the same logic as Common::getTicketStatusForIssue()
      // @see Common::getTicketStatusForIssue()
      ...
      $query1 = [
         'SELECT' => [
            ...
            new QueryExpression("'" . $DB->escape(FormAnswer::getType()) . "' as `itemtype`"),
            $formAnswerTable => [
               'status              as status',
         ...
         'LEFT JOIN' => [
            $formTable => [
               'FKEY' => [
                  $formTable => 'id',
                  $formAnswerTable => Form::getForeignKeyField(),
               ],
            ],
            $itemTicketTable => [
               'FKEY' => [
      ...
      $query2 = [
         'SELECT' => [
            ...
            new QueryExpression("'" . $DB->escape(Ticket::getType()) . "' as `itemtype`"),
               ...
               IF(`$ticketTable`.`global_validation` IN ('" . CommonITILValidation::NONE . "', '" . CommonITILValidation::ACCEPTED . "'),
                  ...
                  IF(`$ticketTable`.`status` IN ('" . CommonITILObject::SOLVED . "', '" . CommonITILObject::CLOSED . "'),
                     ...
                     IF(`$ticketTable`.`global_validation` = '" . CommonITILValidation::WAITING . "',
                        '" . FormAnswer::STATUS_WAITING . "',
                        '" . FormAnswer::STATUS_REFUSED . "'
                     )
                  )
               )
            ) AS `status`"),
   ...
   public function display($options = []) {
      ...
      if (plugin_formcreator_replaceHelpdesk() == EntityConfig::CONFIG_SIMPLIFIED_SERVICE_CATALOG) {
   ...
   public function getTicketsForDisplay($options) {
      ...
      $rows = $DB->request([
         ...
         'WHERE' => [
            ...
            'items_id' => $item->getID() // $item is a FormAnswer
         ],
      ...
      if (count($rows) == 1) {
         ...
         if (isset($options['tickets_id'])) {
            ...
            if (isset($options['tickets_id'])) {
               ...
               Session::setActiveTab(FormAnswer::class, 'Ticket$1');
            }
         }
      }
         ...
         case 'itemtype':
            return Dropdown::showFromArray($name,
                                           [Ticket::class                      => Ticket::getTypeName(1),
                                            FormAnswer::class => FormAnswer::getTypeName(1)],
                                           ['display' => false,
                                            'value'   => $values[$field]]);
         case 'status' :
            $ticket_opts = Ticket::getAllStatusArray(true);
            $ticket_opts = $ticket_opts + FormAnswer::getStatuses();
            unset($ticket_opts[FormAnswer::STATUS_WAITING]);
   ...
   public static function giveItem($itemtype, $option_id, $data, $num) {
      ...
      $rawColumn = 'ITEM_Issue_1_display_id';
      ...
      switch ("$table.$field") {
         ...
         case "glpi_plugin_formcreator_issues.status":
            if ($data['raw']["ITEM_$num"] > 100) {
               ...
               $elements = FormAnswer::getStatuses();
               return FormAnswer::getSpecificValueToDisplay('status', $data['raw']["ITEM_$num"])
   ...
   static function getClosedStatusArray() {
      return [...Ticket::getClosedStatusArray(), FormAnswer::STATUS_ACCEPTED];
   }

   static function getSolvedStatusArray() {
      return [...Ticket::getSolvedStatusArray(), FormAnswer::STATUS_REFUSED];
   }
   ...
   static function getPendingStatusArray() {
      return [Ticket::WAITING, FormAnswer::STATUS_WAITING];
   }
   ...
   static function getAllStatusArray($withmetaforsearch = false) {
      ...
      $form_status = [FormAnswer::STATUS_WAITING, FormAnswer::STATUS_ACCEPTED, FormAnswer::STATUS_REFUSED];
   ...
   static function getValidateCriteria() {
      return ['criteria' => [['link'       => 'AND',
                              'field' => 4,
                              'searchtype' => 'equals',
                              'value'      => FormAnswer::STATUS_WAITING,
                              ],
                            ],
              'reset'    => 'reset'];
   }
   ...
   static function getOldCriteria() {
      return ['criteria' => [
         ...
         ['field' => 4,
            'searchtype' => 'equals',
            'value'      => FormAnswer::STATUS_REFUSED,
            'link'       => 'OR']
         ]],
      ],
      'reset'    => 'reset'];
   ...
   public function prepareInputForAdd($input) {
      ...
      if ($input['itemtype'] == FormAnswer::class) {
   ...
   public static function nbIssues(array $params): array {
      ...
      switch ($params['status']) {
         case 'all':
            $searchCriteria = Issue::getAllCriteria();
            $params['icon']  = "";
            break;

         case 'incoming':
            $searchCriteria = Issue::getNewCriteria();
            $params['icon']  = Ticket::getIcon();
            break;

         case 'waiting':
            $searchCriteria = Issue::getWaitingCriteria();
            $params['icon']  = "fas fa-pause-circle";
            break;

         case 'assigned':
            $searchCriteria = Issue::getAssignedCriteria();
            $params['icon']  = "fas fa-users";
            break;

         case 'validate':
            $params['icon']  = "far fa-eye";
            $searchCriteria = Issue::getValidateCriteria();
            break;

         case 'solved':
            $params['icon']  = "far fa-check-square";
            $searchCriteria = Issue::getSolvedCriteria();
            break;

         case 'closed':
            $params['icon']  = "fas fa-archive";
            $searchCriteria = Issue::getClosedCriteria();
            break;

         case 'old':
            $params['icon']  = "fas fa-archive";
            $searchCriteria = Issue::getOldCriteria();
            break;
      }
