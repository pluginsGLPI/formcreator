...
use GlpiPlugin\Formcreator\Target\AbstractItilTarget;
...
class FormAnswer extends CommonDBTM
{
   ...
   public function canViewItem() {
      ...
      $request = [
         'SELECT' => Form_Validator::getTable() . '.*',
         'FROM' => $this::getTable(),
         'INNER JOIN' => [
            Form::getTable() => [
               'FKEY' => [
                  Form::getTable() => Form::getIndexName(),
                  $this::getTable() => Form::getForeignKeyField(),
               ],
            ],
            Form_Validator::getTable() => [
               'FKEY' => [
                  Form::getTable() => Form::getIndexName(),
                  Form_Validator::getTable() => Form::getForeignKeyField()
               ]
            ]
         ],
   ...
   public function canPurgeItem() {
      return Session::haveRight(Form::$rightname, UPDATE);
   }
   ...
   public function rawSearchOptions() {
      ...
      if ($display_for_form) {
         ...
         $questionsGenerator = Question::getQuestionsFromForm($_SESSION['formcreator']['form_search_answers']);

         foreach ($questionsGenerator as $questionId => $question) {
            $tab[] = [
               'id'            => $optindex,
               'table'         => Answer::getTable(),
               'field'         => 'answer',
   ...
   public function showForm($ID, $options = []) {
      ...
      $style = "<style>";
      // force colums width
      $width_percent = 100 / Section::COLUMNS;
      for ($i = 0; $i < Section::COLUMNS; $i++) {
         $width = ($i+1) * $width_percent;
         $style.= '
         #plugin_formcreator_form.plugin_formcreator_form [data-itemtype = "' . str_replace('\\', '_', Question::getType()) . '"][data-gs-width="' . ($i+1) . '"],
      ...
      echo '<ol>';
      $domain = Form::getTranslationDomain($_SESSION['glpilanguage'], $form->getID());

      // Get fields populated with answers
      $this->loadAnswers();
      $this->answers['plugin_formcreator_forms_id'] = $form->getID();
      $visibility = Fields::updateVisibility($this->answers);

      $sectionsGenerator = Section::getSectionsFromForm($form->getID());
      foreach ($sectionsGenerator as $sectionId => $section) {
         ...
         . ' data-itemtype="' . str_replace('\\', '_', Section::getType()) . '"'
         . ' data-id="' . $sectionId . '"'
         . " $hiddenAttribute"
         . '">';
   ...
   public function prepareInputForUpdate($input) {
      ...
      if (isset($input['refuse_formanswer']) || isset($input['accept_formanswer'])) {
         ...
         $newStatus = isset($input['refuse_formanswer'])
            ? Form_Validator::VALIDATION_STATUS_REFUSED
            : Form_Validator::VALIDATION_STATUS_ACCEPTED;
         if (Plugin::isPluginActive(PLUGIN_FORMCREATOR_ADVANCED_VALIDATION)) {
         ...
         switch ($computedStatus) {
            case Form_Validator::VALIDATION_STATUS_REFUSED:
               $input = $this->refuse($input);
               $skipValidation = true;
               break;

            case Form_Validator::VALIDATION_STATUS_ACCEPTED:
    ...
    *
    * @param Form $form Form to search the unique validator
    * @return null|User|Group
    */
   protected function getUniqueValidator(Form $form): ?CommonDBTM {
      $validValidators = Form_Validator::getValidatorsForForm(
         $form
      );
   ...
   public function generateTarget(): bool {
      ...
      // Generate targets
      $generatedTargets = new Composite(new Item_TargetTicket(), new Ticket_Ticket(), $this);
      foreach ($all_targets as $targets) {
         foreach ($targets as $targetObject) {
            // Check the condition of the target
            if (!Fields::isVisible($targetObject, $this->questionFields)) {
            ...
            if ($targetObject->getTargetType() == AbstractItilTarget::TARGET_TYPE_OBJECT) {
    ...
    *
    * @return Form|null the form used to create this set of answers
    */
   public function getForm(int $formId = null): ?Form {
      if ($this->form !== null) {
         return $this->form;
      }
   ...
   public function getFullForm($richText = false): string {
      ...
      // TODO: code very close to AbstractTarget::parseTags() (factorizable ?)
      ...
      foreach ($questions as $question_line) {
         ...
         if ($last_section != $question_line[$sectionFk]) {
            ...
            if (!Fields::isVisible($currentSection, $this->questionFields)) {
         ...
         if (!Fields::isVisible($fields[$question_line['id']]->getQuestion(), $this->questionFields)) {
            continue;
         }
   ...
   public function post_addItem() {
      ...
      $formId = $this->input[Form::getForeignKeyField()];
    ...
    * @param  TargetInterface $target   Target for which output is being generated
    ...
    */
   public function parseTags(string $content, TargetInterface $target = null, $richText = false): string {
      // Prepare all fields of the form
      $form = $this->getForm();
      $this->getQuestionFields($form->getID());
      $this->deserializeAnswers();
      $domain = Form::getTranslationDomain($form->getID());
      ...
      foreach ($this->questionFields as $questionId => $field) {
         ...
         $name = '';
         $value = '';
         if (Fields::isVisible($question, $this->questionFields)) {
            $name  = $question->fields['name'];
         ...
         if ($question->fields['fieldtype'] === 'file') {
            if (strpos($content, '##answer_' . $questionId . '##') !== false) {
               if ($target !== null && $target instanceof AbstractItilTarget) {
   ...
   protected function validateFormAnswer($input): bool {
      ...
      if ($this->isAnswersValid) {
         foreach ($this->questionFields as $id => $field) {
            ...
            if (Fields::isVisible($field->getQuestion(), $this->questionFields) && !$this->questionFields[$id]->isValid()) {
   ...
   public function isFieldVisible(int $id) : bool {
      ...
      return Fields::isVisible($this->questionFields[$id]->getQuestion(), $this->questionFields);
   }
    ...
    * @return integer
    */
   protected static function getValidationStatus(FormAnswer $formAnswer): int {
      if (Plugin::isPluginActive(PLUGIN_FORMCREATOR_ADVANCED_VALIDATION)) {
   ...
   public function getGeneratedTargets($itemtypes = []): array {
      ...
      if (count($itemtypes) < 1) {
         $itemtypes = Form::getTargetTypes();
      } else {
         $itemtypes = array_intersect(Form::getTargetTypes(), $itemtypes);
      }
   ...
   public function getAggregatedStatus(): ?int {
      $generatedTargets = $this->getGeneratedTargets([TargetTicket::getType()]);
      ...
      $aggregatedStatus = Common::getTicketStatusForIssue($generatedTarget);
      ...
      foreach ($generatedTargets as $generatedTarget) {
         ...
         $ticketStatus = Common::getTicketStatusForIssue($generatedTarget);
         if ($ticketStatus >= FormAnswer::STATUS_WAITING) {
            continue;
         }
   ...
   public function updateStatus(int $status): bool {
      ...
      return $DB->update(Issue::getTable(), [
         'status' => $status,
      ], [
