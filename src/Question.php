...
class Question extends CommonDBChild implements
ExportableInterface,
...
TranslatableInterface
{
    ...
    * workaround use if entity in WHERE when using Question::dropdown
   ...
   public static function showForForm(CommonDBTM $item, $withtemplate = '') {
      $options = [
         'candel'      => false,
         'formoptions' => sprintf('data-itemtype="%s" data-id="%s"', str_replace('\\', '_', $item::getType()), $item->getID()),
      ];
   ...
   public function getDesignHtml() : string {
      ...
      $sectionId = $this->fields[Section::getForeignKeyField()];
      $fieldType = Fields::getFieldClassname($this->fields['fieldtype']);
      ...
      $nb = (new DbUtils())->countElementsInTable(Condition::getTable(), [
         'itemtype' => Question::getType(),
         'items_id' => $this->getID(),
      ]);
    ...
    * @param FormAnswer $value   Values all fields of the form
    * @param bool $isVisible is the question visible by default ?
    *
    * @return string
    */
   public function getRenderedHtml($domain, $canEdit = true, ?FormAnswer $form_answer = null, $isVisible = true): string {
      if ($this->isNewItem()) {
         return '';
      }
   ...
   private function checkBeforeSave($input) : array {
      ...
      $form = Common::getForm();
      $section = Section::getById($input[Section::getForeignKeyField()]);
   ...
   public function prepareInputForAdd($input) {
      ...
      if (!isset($input['width'])) {
         $input['width'] = Section::COLUMNS - $input['col'];
      }
      // Get next row
      if ($this->useAutomaticOrdering) {
         $maxRow = Common::getMax($this, [
   ...
   public function change($input): bool {
      ...
      if (isset($input['width'])) {
         ...
         if ($input['width'] > (Section::COLUMNS - $x)) {
            return false;
         }
         $width = $input['width'];
      }
   ...
   public function pre_deleteItem() {
      $success = (new Condition())->deleteByCriteria([
         'itemtype' => self::class,
         'items_id' => $this->getID(),
      ]);
      if (!$success) {
   ...
   public function post_purgeItem() {
      ...
      $table = self::getTable();
      $condition_table = Condition::getTable();
   ...
   public static function import(Linker $linker, array $input = [], int $containerId = 0) {
      ...
      if ($itemId !== false) {
         $input['id'] = $itemId;
         $item->field = Fields::getFieldInstance(
            $input['fieldtype'],
            $item
         );
         $item->update($input);
   ...
   public static function getQuestionsFromForm($formId, $crit = []): \Generator {
      ...
      $table_question = Question::getTable();
      $table_section  = Section::getTable();
      $sectionFk = Section::getForeignKeyField();
   ...
   public static function getQuestionDataById($table, $id) {
      ...
      $validTargets = [
         Condition::getTable(),
         QuestionRegex::getTable(),
         QuestionRange::getTable(),
      ];
   ...
   private function loadField($fieldType): bool {
      ...
      $this->field = Fields::getFieldInstance($fieldType, $this);
      if ($this->field === null) {
         return false;
      }
      return true;
    ...
    */
   public function getSubField(): FieldInterface {
      if ($this->isNewItem()) {
         return null;
      }

      if ($this->field === null) {
         $this->field = Fields::getFieldInstance(
