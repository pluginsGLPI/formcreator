...
class Fields
{
   ...
   public static function getTypes() {
      ...
      foreach (glob(dirname(__FILE__).'/field/*field.class.php') as $class_file) {
         ...
         if (Fields::fieldTypeExists($matches[1])) {
   ...
   public static function getClasses() {
      $classes = [];
      $path = dirname(__FILE__) . '/Field/*Field.php';
      foreach (glob($path) as $class_file) {
         $matches = null;
         preg_match("#Field/(.+)Field\.php$#", $class_file, $matches);
   ...
   public static function getNames() {
      ...
      foreach (Fields::getClasses() as $field_type => $classname) {
    ...
    * @param   FieldInterface[] $fields     Array of fields instances (question id => instance)
    * @return  boolean                 If true the question should be visible
    */
   public static function isVisible(ConditionnableInterface $item, $fields) {
      /** @var CommonDBTM $item */
      ...
      $getParentVisibility = function() use ($item, $fields) {
         ...
         if ($item instanceof CommonDBChild) {
            ...
            if (in_array(ConditionnableInterface::class, $interfaces)) {
               if ($parent = $item->getItem(true, false)) {
                  $parentItemtype = $parent->getType();
                  $parentId = $parent->getID();
                  if ($parent->getType() == Form::class) {
      ...
      if ($item->fields['show_rule'] == Condition::SHOW_RULE_ALWAYS) {
      ...
      if (count($conditions) < 1) {
         switch ($item->fields['show_rule']) {
            case Condition::SHOW_RULE_HIDDEN:
               self::$visibility[$itemtype][$itemId] = false;
               return self::$visibility[$itemtype][$itemId];
               break;

            case Condition::SHOW_RULE_SHOWN:
      ...
      foreach ($conditions as $condition) {
         ...
         if (in_array($condition->fields['show_condition'], [Condition::SHOW_CONDITION_QUESTION_VISIBLE, Condition::SHOW_CONDITION_QUESTION_INVISIBLE])) {
            switch ($condition->fields['show_condition']) {
               case Condition::SHOW_CONDITION_QUESTION_VISIBLE:
                  ...
                  break;
               case Condition::SHOW_CONDITION_QUESTION_INVISIBLE:
                  if (!$conditionField->isPrerequisites()) {
                     self::$visibility[$itemtype][$itemId] = false;
                     return self::$visibility[$itemtype][$itemId];
                  }
            ...
            if (self::isVisible($conditionField->getQuestion(), $fields)) {
               switch ($condition->fields['show_condition']) {
                  case Condition::SHOW_CONDITION_NE :
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = true;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_EQ :
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_GT:
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_LT:
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_GE:
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_LE:
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
                  ...
                  case Condition::SHOW_CONDITION_REGEX:
                     if (!$conditionField->isPrerequisites()) {
                        self::$visibility[$itemtype][$itemId] = false;
                        return self::$visibility[$itemtype][$itemId];
                     }
         ...
         $expression[] = Condition::getEnumShowLogic()[$condition->fields['show_logic']];
      ...
      if ($item->fields['show_rule'] == Condition::SHOW_RULE_SHOWN) {
         // Reverse condition
   ...
   public static function updateVisibility($input) {
      ...
      $submitShow = Fields::isVisible($form, $fields);

      // Get the visibility result of questions
      $questionToShow = [];
      foreach ($fields as $id => $field) {
         $questionToShow[$id] = Fields::isVisible($field->getQuestion(), $fields);
      }
      ...
      foreach ($sectionsGenerator as $sectionId => $section) {
         $sectionToShow[$sectionId] = Fields::isVisible($section, $fields);
      }

      return [
         Question::class => $questionToShow,
         Section::class  => $sectionToShow,
         Form::class     => $submitShow,
      ];
   }
   ...
   public static function fieldTypeExists(string $type): bool {
      ...
      return is_subclass_of($className, AbstractField::class, true);
   }
    ...
    * @param Question $question question representing the field
    * @return null|AbstractField
    */
   public static function getFieldInstance(string $type, Question $question): ?AbstractField {
