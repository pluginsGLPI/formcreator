...
class Form_Language extends CommonDBChild
implements ExportableInterface
{
   ...
   public function post_updateItem($history = 1) {
      ...
      $formFk = Form::getForeignKeyField();
      $domain = Form::getTranslationDomain($this->fields['name'], $this->fields[$formFk]);
      $TRANSLATE->clearCache($domain, $this->fields['name']);
   }

   public function pre_deleteItem() {
      // Delete translation file
      $file = Form::getTranslationFile(
         $this->fields[Form::getForeignKeyField()],
         $this->fields['name']
      );
      if (file_exists($file)) {
         return unlink($file);
   ...
   public function showForm($ID, $options = []) {
      if (!$this->isNewID($ID)) {
         ...
         $item = Common::getForm();
         $item->getFromDB($this->fields[Form::getForeignKeyField()]);
      } else {
         $this->getEmpty();
         if (!isset($options['parent']) || empty($options['parent'])) {
            return false;
         }
         $this->fields[Form::getForeignKeyField()] = $options['parent']->getID();
         $item = $options['parent'];
      }
      ...
      $used = [];
      $rows = $this->find([Form::getForeignKeyField() => $item->getID()]);
      foreach ($rows as $row) {
      ...
      echo "</tr></tr>";
      echo '<td>';
      echo Html::hidden(Form::getForeignKeyField(), ['value' => $item->getID()]);
      echo '</td>';
      echo '</tr>';
   ...
   public function showNewTranslation($options = []) {
      ...
      echo '<div data-itemtype="' . str_replace('\\', '_', Form_Language::getType()) . '" data-id="' . $this->getID() . '">';
      ...
      echo Translation::getEditorFieldsHtml($this);
      echo '</tr>';
      echo "<tr class='tab_bg_1'><td>";
      echo "</td><td width='50%'>&nbsp;</td></tr>";
   ...
   public function showTranslationEntry($input) : void {
      ...
      echo Translation::getEditorFieldsHtml($this, $input['plugin_formcreator_translations_id']);
   ...
   public static function showForForm(CommonDBTM $item, $withtemplate = '') {
      ...
      foreach ($iterator as $data) {
         ...
         $url = Form_Language::getFormURLWithID($id);
         echo "<td $onhover>";
         echo '<a href="' . $url . '">';
