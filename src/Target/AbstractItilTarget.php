...
namespace GlpiPlugin\Formcreator\Target;
...
use GlpiPlugin\Formcreator\TranslatableInterface;
use Group;
use Group_User;
use Html;
use ITILCategory;
...
abstract class AbstractItilTarget extends AbstractTarget implements
ExportableInterface,
ItilTargetInterface,
...
TranslatableInterface
{
    ...
    * @param FormAnswer $formanswer Answers to the form used to populate the target
    * @return array
    */
   protected function setTargetCategory(array $data, FormAnswer $formanswer) : array {
      global $DB;
      ...
      switch ($this->fields['category_rule']) {
         ...
         case self::CATEGORY_RULE_LAST_ANSWER:
            ...
            $answers = $DB->request([
               ...
               'FROM' => Answer::getTable() . ' AS answer',
               'JOIN' => [
                  Question::getTable() . ' AS question' => [
                     'ON' => [
            ...
            foreach ($answers as $answer) {
               ...
               $question = Question::getById($answer[Question::getForeignKeyField()]);
    ...
    */
   protected function prepareActors(Form $form, FormAnswer $formanswer) {
      global $DB;
      ...
      foreach ($rows as $actor) {
         ...
         if ($actor['actor_type'] == Target_Actor::ACTOR_TYPE_VALIDATOR && !$form->fields['validation_required']) {
            continue;
         }

         switch ($actor['actor_type']) {
            case Target_Actor::ACTOR_TYPE_AUTHOR :
               $userIds = [$formanswer->fields['requester_id']];
               break;
            case Target_Actor::ACTOR_TYPE_VALIDATOR :
               $userIds = [$_SESSION['glpiID']];
               break;
            case Target_Actor::ACTOR_TYPE_PERSON :
            case Target_Actor::ACTOR_TYPE_GROUP :
            case Target_Actor::ACTOR_TYPE_SUPPLIER :
               $userIds = [$actor['actor_value']];
               break;
            case Target_Actor::ACTOR_TYPE_QUESTION_PERSON :
            case Target_Actor::ACTOR_TYPE_QUESTION_GROUP :
            case Target_Actor::ACTOR_TYPE_QUESTION_SUPPLIER :
               $answer  = new Answer();
               ...
               break;
            case Target_Actor::ACTOR_TYPE_QUESTION_ACTORS:
               $answer  = new Answer();
               $actorValue = $actor['actor_value'];
               $formanswerId = $formanswer->getID();
               $answer->getFromDBByCrit([
               ...
               break;
            case Target_Actor::ACTOR_TYPE_GROUP_FROM_OBJECT:
            case Target_Actor::ACTOR_TYPE_TECH_GROUP_FROM_OBJECT:
               ...
               $question->getFromDB($answer->fields[Question::getForeignKeyField()]);
               ...
               if ($actor['actor_type'] == Target_Actor::ACTOR_TYPE_TECH_GROUP_FROM_OBJECT) {
            ...
            case Target_Actor::ACTOR_TYPE_AUTHORS_SUPERVISOR:
         ...
         switch ($actor['actor_type']) {
            ...
            case Target_Actor::ACTOR_TYPE_QUESTION_SUPPLIER :
               foreach ($userIds as $userId) {
                  $this->addActor(Target_Actor::ACTOR_ROLE_SUPPLIER, $userId, $notify);
               }
               break;
         }
      }
   ...
   protected  function showDueDateSettings() {
      ...
      $questionsGenerator = Question::getQuestionsFromForm(
         $this->getForm()->getID(),
         [
...
SCRIPT;
   ...
   protected function showActorsSettings() {
      ...
      $actors = [
         Target_Actor::ACTOR_ROLE_REQUESTER => [],
         Target_Actor::ACTOR_ROLE_OBSERVER => [],
         Target_Actor::ACTOR_ROLE_ASSIGNED => [],
      ];
      $result = $DB->request([
      ...
      . ' data-itemtype="' . str_replace('\\', '_', $this->getType()) . '"'
      . ' data-id="' . $this->getID() . '"'
      . '>';
    ...
    * @param FormAnswer $formanswer    Answers previously saved
    * @return array updated data of the target object
    */
   protected function setTargetDueDate($data, FormAnswer $formanswer) {
      global $DB;
         ...
         $request = [
            ...
            'WHERE' => [
               'AND' => [
                  ...
                  Question::getForeignKeyField() => $this->fields['due_date_question'],
               ],
            ],
         ];
   ...
   public function pre_purgeItem() {
      ...
      $targetItemActor = new Target_Actor();
    ...
    *
    * @param FormAnswer $formanswer the source formanswer
    * @param int $targetId ID of the generated target
    * @return void
    */
   protected function saveTags(FormAnswer $formanswer, $targetId) {
      global $DB;
      ...
      if (($this->fields['tag_type'] == self::TAG_TYPE_QUESTIONS
         ...
         $formAnswerFk = FormAnswer::getForeignKeyField();
   ...
   protected function showActorSettingsForType($actorType, array $actors) {
      ...
      switch ($actorType) { // Values from CommonITILObject::getSearchOptionsActors()
         case CommonITILActor::REQUESTER:
            $type = 'requester';
            unset($dropdownItems[Target_Actor::ACTOR_TYPE_SUPPLIER]);
            unset($dropdownItems[Target_Actor::ACTOR_TYPE_QUESTION_SUPPLIER]);
            $changeActorJSFunction = 'plugin_formcreator.changeActor("requester", this.value)';
            $actorRole = Target_Actor::ACTOR_ROLE_REQUESTER;
            break;
         case CommonITILActor::OBSERVER:
            $type = 'watcher';
            $changeActorJSFunction = 'plugin_formcreator.changeActor("watcher", this.value)';
            $actorRole = Target_Actor::ACTOR_ROLE_OBSERVER;
            break;
         case CommonITILActor::ASSIGN:
            $type = 'assigned';
            unset($dropdownItems[Target_Actor::ACTOR_TYPE_AUTHORS_SUPERVISOR]);
            $changeActorJSFunction = 'plugin_formcreator.changeActor("assigned", this.value)';
            $actorRole = Target_Actor::ACTOR_ROLE_ASSIGNED;
            break;
      }
      ...
      User::dropdown([
         'name' => 'actor_value_' . Target_Actor::ACTOR_TYPE_PERSON,
         'right' => 'all',
         'all'   => 0,
      ]);
      echo '</div>';
      ...
      Group::dropdown([
         'name' => 'actor_value_' . Target_Actor::ACTOR_TYPE_GROUP,
      ]);
      echo '</div>';
      ...
      Question::dropdownForForm(
         ...
         'actor_value_' . Target_Actor::ACTOR_TYPE_QUESTION_PERSON,
         0,
         [
            'used' => $used,
         ]
      ...
      $request = $DB->request([
         ...
         'WHERE' => [
            ...
            'items_id'   => $this->getID(),
            'actor_role' => $actorRole,
            'actor_type' => Target_Actor::ACTOR_TYPE_QUESTION_GROUP,
         ]
      ]);
      $used = [];
      foreach ($request as $row) {
      ...
      Question::dropdownForForm(
         ...
         'actor_value_' .  Target_Actor::ACTOR_TYPE_QUESTION_GROUP,
         0,
         [
            'used' => $used,
         ]
      ...
      $request = $DB->request([
         ...
         'WHERE' => [
            ...
            'items_id'   => $this->getID(),
            'actor_role' => $actorRole,
            'actor_type' => Target_Actor::ACTOR_TYPE_GROUP_FROM_OBJECT,
         ]
      ]);
      $used = [];
      foreach ($request as $row) {
      ...
      Question::dropdownForForm(
         ...
         'actor_value_' .  Target_Actor::ACTOR_TYPE_GROUP_FROM_OBJECT,
         0,
         [
            'used' => $used,
         ]
      ...
      $request = $DB->request([
         ...
         'WHERE' => [
            ...
            'items_id'   => $this->getID(),
            'actor_role' => $actorRole,
            'actor_type' => Target_Actor::ACTOR_TYPE_TECH_GROUP_FROM_OBJECT,
         ]
      ]);
      $used = [];
      foreach ($request as $row) {
      ...
      Question::dropdownForForm(
         ...
         'actor_value_' .  Target_Actor::ACTOR_TYPE_TECH_GROUP_FROM_OBJECT,
         0,
         [
            'used' => $used,
         ]
      ...
      $request = $DB->request([
         ...
         'WHERE' => [
            ...
            'items_id'   => $this->getID(),
            'actor_role' => $actorRole,
            'actor_type' => Target_Actor::ACTOR_TYPE_QUESTION_ACTORS,
         ]
      ]);
      $used = [];
      foreach ($request as $row) {
      ...
      Question::dropdownForForm(
         ...
         'actor_value_' . Target_Actor::ACTOR_TYPE_QUESTION_ACTORS,
         0,
         [
            'used' => $used,
         ]
      ...
      if ($actorType == CommonITILActor::ASSIGN) {
         ...
         $request = $DB->request([
            ...
            'WHERE' => [
               ...
               'items_id'   => $this->getID(),
               'actor_role' => $actorRole,
               'actor_type' => Target_Actor::ACTOR_TYPE_SUPPLIER,
            ]
         ]);
         $used = [];
         foreach ($request as $row) {
         ...
         Supplier::dropdown([
            'name' => 'actor_value_' . Target_Actor::ACTOR_TYPE_SUPPLIER,
            'used' => $used,
         ]);
         echo '</div>';
         ...
         $request = $DB->request([
            ...
            'WHERE' => [
               ...
               'items_id'   => $this->getID(),
               'actor_role' => $actorRole,
               'actor_type' => Target_Actor::ACTOR_TYPE_QUESTION_SUPPLIER,
            ]
         ]);
         $used = [];
         foreach ($request as $row) {
         ...
         Question::dropdownForForm(
            ...
            'actor_value_' . Target_Actor::ACTOR_TYPE_QUESTION_SUPPLIER,
            0,
            [
               'used' => $used,
            ]
      ...
      foreach ($actors[$actorRole] as $id => $values) {
         echo '<div data-itemtype="' . str_replace('\\', '_', Target_Actor::getType()) . '" data-id="' . $id . '">';
    ...
    * @return array
    */
   public function getDefaultData(FormAnswer $formanswer): array {
      ...
      if (isset($predefined_fields['_users_id_requester'])) {
         $this->addActor(Target_Actor::ACTOR_ROLE_REQUESTER, $predefined_fields['_users_id_requester'], true);
         unset($predefined_fields['_users_id_requester']);
      }
      if (isset($predefined_fields['_users_id_observer'])) {
         $this->addActor(Target_Actor::ACTOR_ROLE_OBSERVER, $predefined_fields['_users_id_observer'], true);
         unset($predefined_fields['_users_id_observer']);
      }
      if (isset($predefined_fields['_users_id_assign'])) {
         $this->addActor(Target_Actor::ACTOR_ROLE_ASSIGNED, $predefined_fields['_users_id_assign'], true);
         unset($predefined_fields['_users_id_assign']);
      }

      if (isset($predefined_fields['_groups_id_requester'])) {
         $this->addGroupActor(Target_Actor::ACTOR_ROLE_REQUESTER, $predefined_fields['_groups_id_requester']);
         unset($predefined_fields['_groups_id_requester']);
      }
      if (isset($predefined_fields['_groups_id_observer'])) {
         $this->addGroupActor(Target_Actor::ACTOR_ROLE_OBSERVER, $predefined_fields['_groups_id_observer']);
         unset($predefined_fields['_groups_id_observer']);
      }
      if (isset($predefined_fields['_groups_id_assign'])) {
         $this->addGroupActor(Target_Actor::ACTOR_ROLE_ASSIGNED, $predefined_fields['_groups_id_assign']);
   ...
   protected function prepareUploadsFromTextarea(array $data, FormAnswer $formanswer): array {
   ...
   protected function prepareUploadedFiles(array $data): array {
      ...
      // TODO: replace Common::getDocumentsFromTag by Toolbox::getDocumentsFromTag
      // when is merged https://github.com/glpi-project/glpi/pull/9335
      foreach (Common::getDocumentsFromTag($data['content']) as $document) {
         $prefix = uniqid('', true);
    ...
    * @return array
    */
   public static function findForFormAnswer(FormAnswer $formAnswer): array {
      global $DB;
      ...
      $iterator = $DB->request([
         ...
         'WHERE' => [
            "$relationTable.itemtype" => FormAnswer::getType(),
