...
use GlpiPlugin\Formcreator\ConditionnableTrait;
use GlpiPlugin\Formcreator\ExportableInterface;
use GlpiPlugin\Formcreator\ExportableTrait;
...
use GlpiPlugin\Formcreator\TranslatableTrait;
use Html;
use Plugin;
...
abstract class AbstractTarget extends CommonDBChild implements
   ExportableInterface,
   TargetInterface,
   ...
   TranslatableInterface
{
   ...
   static public $itemtype = Form::class;
    ...
    */
   abstract public function save(FormAnswer $formanswer): ?CommonDBTM;
   ...
   public static function getEnumShowRule(): array {
      return [
         Condition::SHOW_RULE_ALWAYS => __('Always generated', 'formcreator'),
         Condition::SHOW_RULE_HIDDEN => __('Disabled unless', 'formcreator'),
         Condition::SHOW_RULE_SHOWN  => __('Generated unless', 'formcreator'),
      ];
   }
   ...
   public function pre_purgeItem() {
      // delete conditions
      if (! (new Condition())->deleteByCriteria([
    ...
    * @param FormAnswer $formAnswer form answer to render
    * @param bool $richText Disable rich text output
    * @return string
    */
   protected function prepareTemplate($template, FormAnswer $formAnswer, $richText = false) {
    ...
    * @return void
    */
   protected function appendFieldsData(FormAnswer $formanswer, &$input): void {
      global $DB;

      //get all Answer
      //from FormAnswer
      //where linked question have 'fields' type
      ...
      foreach ($result as $line) {
         $formQuestion = Question::getById($line['plugin_formcreator_questions_id']);
   ...
   protected function convertTags($input) {
      $questionsGenerator = Question::getQuestionsFromForm($this->getForm()->getID());
   ...
   final public static function showConditions(self $item) {
      ...
      $options['formoptions'] = sprintf('data-itemtype="%s" data-id="%s"', str_replace('\\', '_', self::getType()), $item->getID());
   ...
   public function getForm() {
      if ($this->form === null) {
         $form = Common::getForm();
         if (!$form->getFromDB($this->fields[Form::getForeignKeyField()])) {
            return null;
         }
         $this->form = $form;
      }
    ...
    */
   protected function setTargetEntity($data, FormAnswer $formanswer, $requesters_id) {
      global $DB;
