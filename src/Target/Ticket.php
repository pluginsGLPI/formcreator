...
class Ticket extends AbstractItilTarget
{
   ...
   public function showForm($ID, $options = []) {
      $options = [
         'candel'      => false,
         'formoptions' => sprintf('data-itemtype="%s"', str_replace('\\', '_', $this::getType())),
      ];
   ...
   public static function showProperties(self $item) {
      ...
      . ' data-itemtype="' . str_replace('\\', '_', self::class) . '"'
      . '>';
   ...
   public static function showPluginFields(self $item) {
      ...
      $canEdit = Session::haveRight(Form::$rightname, UPDATE);
   ...
   protected function showCompositeTicketSettings($rand) {
      ...
      $elements = [
         self::class => __('An other destination of this form', 'formcreator'),
         GlpiTicket::class   => __('An existing ticket', 'formcreator'),
         Question::class     => __('A ticket from an answer to a question'),
      ];
      ...
      $rows = $DB->request([
         'FROM'   => Item_TargetTicket::getTable(),
         'WHERE'  => [
      ...
      echo GlpiTicket::dropdown($linkparam);
      echo '</span>';
   ...
   public function prepareInputForAdd($input) {
      ...
      if ($input['type_rule'] == self::REQUESTTYPE_SPECIFIC) {
         if (!isset($input['type_question']) || !in_array($input['type_question'], [GlpiTicket::INCIDENT_TYPE, GlpiTicket::DEMAND_TYPE])) {
            $input['type_question'] = GlpiTicket::INCIDENT_TYPE;
         }
      }
   ...
   protected function getTargetTemplate(array $data): int {
      ...
      if ($targetItemtype::isNewID($this->fields[$targetTemplateFk]) && !ITILCategory::isNewID($data['itilcategories_id'])) {
         ...
         if ($row = $rows->current()) { // assign ticket template according to resulting ticket category and ticket type
            return ($data['type'] == GlpiTicket::INCIDENT_TYPE
   ...
   /**
    * Prepare input for target creation
    *
    * @param  FormAnswer $formanswer    Answers previously saved
    *
    * @return array input data to craete the target
    */
   public function prepareTargetInput(FormAnswer $formanswer): array {
      ...
      if (count($this->requesters['_users_id_requester']) == 0) {
         $this->addActor(Target_Actor::ACTOR_ROLE_REQUESTER, $formanswer->fields['requester_id'], true);
    ...
    */
   public function save(FormAnswer $formanswer): ?CommonDBTM {
      // Create the target ticket
      $data = $this->prepareTargetInput($formanswer);
      $data['_auto_import'] = true;
      $data['_skip_sla_assign'] = true;
      ...
      if ($form->validationRequired() && $this->fields['validation_followup']) {
         ...
         $use_mailing = Common::isNotificationEnabled();
         Common::setNotification(false);
         ...
         Common::setNotification($use_mailing);
      }
   ...
   protected function setTargetType(array $data, FormAnswer $formanswer) {
      global $DB;
   ...
   protected function showTargetType($rand) {
      ...
      echo '</td>';
      echo '<td width="15%">';
      echo '<span id="requesttype_question_title" style="display: none">' . Question::getTypeName(1) . '</span>';
      ...
      GlpiTicket::dropdownType('_type_specific',
         [
   ...
   protected function showAssociateSettings($rand) {
      ...
      $itemTargetTicket = new Item_TargetTicket();
      $rows = $itemTargetTicket->find([
         self::getForeignKeyField() => $this->getID(),
         [
            'NOT' => ['itemtype' => [self::class, GlpiTicket::class]],
         ],
      ]);
      foreach ($rows as $row) {
         $options['items_id'][$row['itemtype']][$row['id']] = $row['items_id'];
      }
      Item_Ticket::itemAddForm(new GlpiTicket(), $options);
      echo '</div>';
    ...
    * @return array
    */
   protected function setTargetAssociatedItem(array $data, FormAnswer $formanswer) : array {
      global $DB, $CFG_GLPI;
      ...
      switch ($this->fields['associate_rule']) {
         ...
         case self::ASSOCIATE_RULE_LAST_ANSWER:
            ...
            foreach ($answers as $answer) {
               ...
               $question = new Question();
               $question->getFromDB($answer[Question::getForeignKeyField()]);
               /** @var  GlpiPlugin\Formcreator\Field\DropdownField */
               $field = $question->getSubField();
   ...
   public static function import(Linker $linker, array $input = [], int $containerId = 0) {
      ...
      $questions = $linker->getObjectsByType(Question::class);
      if ($questions !== false) {
      ...
      foreach ($questionLinks as $field => $fieldSetting) {
         ...
         $question = $linker->getObject($input[$fieldSetting['field']], Question::class);
         if ($question === false) {
            $typeName = strtolower(self::getTypeName());
            throw new \GlpiPlugin\Formcreator\Exception\ImportFailureException(sprintf(__('Failed to add or update the %1$s %2$s: a question is missing and is used in a parameter of the target', 'formceator'), $typeName, $input['name']));
         }
      ...
      $subItems = [
         '_actors'            => Target_Actor::class,
         '_ticket_relations'  => Item_TargetTicket::class,
