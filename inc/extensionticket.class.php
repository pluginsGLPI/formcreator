 ...
 * Ticket Class
 * */
class PluginFormcreatorExtensionTicket extends CommonITILObject {
    ...
    function canAssignToMe() {
        ...
        return (Session::haveRight(self::$rightname, self::STEAL) || (Session::haveRight(self::$rightname, self::OWN) && ($this->countUsers(CommonITILActor::ASSIGN) == 0)));
    }
    ...
    static function canUpdate() {
        ...
        return Session::haveRightsOr(self::$rightname, array(UPDATE, self::ASSIGN, self::STEAL, self::OWN));
    }
    ...
    static function canView() {
        ...
        return (Session::haveRightsOr(self::$rightname, array(self::READALL, self::READMY, UPDATE, self::READASSIGN,
                    self::READGROUP, self::OWN)) || Session::haveRightsOr('ticketvalidation', TicketValidation::getValidateRights()));
    }
    ...
    function canSolve() {
        ...
        return ((Session::haveRight(self::$rightname, UPDATE) || $this->isUser(CommonITILActor::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::ASSIGN, $_SESSION["glpigroups"]))) && self::isAllowedStatus($this->fields['status'], self::SOLVED)
    ...
    function canApprove() {
        ...
        return ((($this->fields["users_id_recipient"] === Session::getLoginUserID()) && Session::haveRight('ticket', Ticket::SURVEY)) || $this->isUser(CommonITILActor::REQUESTER, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::REQUESTER, $_SESSION["glpigroups"])));
    }
    ...
    function canUpdateItem() {
        ...
        if (($this->numberOfFollowups() == 0) && ($this->numberOfTasks() == 0) && ($this->isUser(CommonITILActor::REQUESTER, Session::getLoginUserID()) || ($this->fields["users_id_recipient"] === Session::getLoginUserID()))) {
            return true;
        }
    ...
    function canDeleteItem() {
        ...
        if (($this->isUser(CommonITILActor::REQUESTER, Session::getLoginUserID()) || ($this->fields["users_id_recipient"] === Session::getLoginUserID())) && ($this->numberOfFollowups() == 0) && ($this->numberOfTasks() == 0) && ($this->fields["date"] == $this->fields["date_mod"])) {
            return true;
        }
    ...
    function getDefaultActor($type) {
        ...
        if ($type == CommonITILActor::REQUESTER) {
            if (Session::haveRight(self::$rightname, CREATE) && $_SESSION['glpiset_default_requester']) {
                return Session::getLoginUserID();
            }
        }
        return 0;
    ...
    function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
        ...
        if (static::canView()) {
            ...
            if ($_SESSION['glpishow_count_on_tabs']) {
                switch ($item->getType()) {
                    case 'User' :
                        $nb = countElementsInTable(array('glpi_tickets', 'glpi_tickets_users'), getEntitiesRestrictRequest("", 'glpi_tickets') .
                    ...
                    case 'Supplier' :
                        $nb = countElementsInTable(array('glpi_tickets', 'glpi_suppliers_tickets'), getEntitiesRestrictRequest("", 'glpi_tickets') .
                                "AND `glpi_suppliers_tickets`.`tickets_id` = `glpi_tickets`.`id`
                                                AND `glpi_suppliers_tickets`.`suppliers_id` = '" . $item->getID() . "'");
                        break;

                    case 'SLT' :
                        $nb = countElementsInTable('glpi_tickets', "`slts_tto_id` = '" . $item->getID() . "'
                                               OR `slts_ttr_id` = '" . $item->getID() . "'");
                        break;

                    case 'Group' :
                        $nb = countElementsInTable(array('glpi_tickets', 'glpi_groups_tickets'), getEntitiesRestrictRequest("", 'glpi_tickets') .
        ...
        switch ($item->getType()) {
            case __CLASS__ :
                switch ($tabnum) {
                    ...
                    case 3 :
                        $satisfaction = new TicketSatisfaction();
                        if (($item->fields['status'] == self::CLOSED) && $satisfaction->getFromDB($_GET["id"])) {

                            $duration = Entity::getUsedConfig('inquest_duration', $item->fields['entities_id']);
                            $date2 = strtotime($satisfaction->fields['date_begin']);
                            if (($duration == 0) || (strtotime("now") - $date2) <= $duration * DAY_TIMESTAMP) {
    ...
    function prepareInputForUpdate($input) {
        ...
        if (!Session::isCron() && (!Session::haveRight(self::$rightname, UPDATE)
                // Closed tickets
                || in_array($this->fields['status'], $this->getClosedStatusArray()))
        ) {
        ...
        if (count($changes)) {
            ...
            $input = $rules->processAllRules(Toolbox::stripslashes_deep($input), Toolbox::stripslashes_deep($input), array('recursive' => true,
                'entities_id' => $entid), array('condition' => RuleTicket::ONUPDATE,
    ...
    function sltAffect($type, &$input, $manual_slts_id) {
        ...
        if (isset($this->fields['id']) && $this->fields['id'] > 0) {
            if (!isset($manual_slts_id[$type]) && isset($input[$sltField]) && ($input[$sltField] > 0) && ($input[$sltField] != $this->fields[$sltField])) {
            ...
            if (isset($input[$sltField]) && ($input[$sltField] > 0) && ($input[$sltField] != $this->fields[$sltField])) {
                ...
                $slt_data = $this->getDatasToAddSLT($input[$sltField], $this->fields['entities_id'], $date, $type);
                if (count($slt_data)) {
            ...
            if (!isset($manual_slts_id[$type]) && isset($input[$dateField]) && ($input[$dateField] != 'NULL')) {
                // Valid due date
            ...
            if (isset($input[$sltField]) && ($input[$sltField] > 0)) {
                // Get datas to initialize SLT and set it
                $slt_data = $this->getDatasToAddSLT($input[$sltField], $input['entities_id'], $input['date'], $type);
                if (count($slt_data)) {
                    foreach ($slt_data as $key => $val) {
                        $input[$key] = $val;
                    }
    ...
    function pre_updateInDB() {
        ...
        if (($this->fields['takeintoaccount_delay_stat'] == 0) && (Session::haveRight("task", CommonITILTask::ADDALLITEM) || Session::haveRightsOr('followup', array(TicketFollowup::ADDALLTICKET,
                    TicketFollowup::ADDMYTICKET,
                    TicketFollowup::ADDGROUPTICKET)) || $this->isUser(CommonITILActor::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::ASSIGN, $_SESSION['glpigroups'])))) {
    ...
    function post_updateItem($history = 1) {
        ...
        if (count($this->updates)) {
            ...
            if ((in_array("solutiontypes_id", $this->updates) || in_array("solution", $this->updates)) && (isset($this->input["status"]) && (in_array($this->input["status"], $this->getSolvedStatusArray()) || in_array($this->input["status"], $this->getClosedStatusArray())))) { // auto close case
...
//          $toadd['_no_notif'] = true;
        ...
        if (isset($_SESSION['glpiis_ids_visible']) && !$_SESSION['glpiis_ids_visible']) {
            Session::addMessageAfterRedirect(sprintf(__('%1$s (%2$s)'), __('Your ticket has been registered, its treatment is in progress.'), sprintf(__('%1$s: %2$s'), __('Ticket'), "<a href='" . $CFG_GLPI["root_doc"] .
    ...
    function manageValidationAdd($input) {
        ...
        if (isset($input["_add_validation"])) {
            ...
            if (count($validations_to_send)) {
                ...
                if (Session::isCron() || isset($input["_auto_import"]) || isset($input["_rule_process"]) || $validation->can(-1, CREATE, $values)) { // cron or allowed user
                    $add_done = false;
    ...
    function updateDateMod($ID, $no_stat_computation = false, $users_id_lastupdater = 0) {
        ...
        if ($this->getFromDB($ID)) {
            if (!$no_stat_computation && (Session::haveRight('task', CommonITILTask::ADDALLITEM) || Session::haveRightsOr('followup', array(TicketFollowup::ADDALLTICKET,
                        TicketFollowup::ADDMYTICKET,
                        TicketFollowup::ADDGROUPTICKET)) || $this->isUser(CommonITILActor::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::ASSIGN, $_SESSION['glpigroups'])) || isCommandLine())) {
    ...
    function canAddFollowups() {
        ...
        return ((Session::haveRight("followup", TicketFollowup::ADDMYTICKET) && ($this->isUser(CommonITILActor::REQUESTER, Session::getLoginUserID()) || ($this->fields["users_id_recipient"] === Session::getLoginUserID()))) || Session::haveRight('followup', TicketFollowup::ADDALLTICKET) || (Session::haveRight('followup', TicketFollowup::ADDGROUPTICKET) && isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::REQUESTER, $_SESSION['glpigroups'])) || $this->isUser(CommonITILActor::ASSIGN, Session::getLoginUserID()) || (isset($_SESSION["glpigroups"]) && $this->haveAGroup(CommonITILActor::ASSIGN, $_SESSION['glpigroups'])));
    }
    ...
    function getSearchOptions() {
        ...
        if (!Session::isCron() // no filter for cron
                && (!isset($_SESSION['glpiactiveprofile']['interface']) || ($_SESSION['glpiactiveprofile']['interface'] == 'helpdesk'))) {
    ...
    function showFormHelpdesk($ID, $ticket_template = false) {
        ...
        $default_values = array('_users_id_requester_notif'
            ...
            '_add_validation' => 0,
            'type' => Entity::getUsedConfig('tickettype', $_SESSION['glpiactive_entity'], '', Ticket::INCIDENT_TYPE),
            '_right' => "id",
            '_filename' => array(),
            '_tag_filename' => array());
...
//       "<div id='uploadfiles'><input type='file' name='filename[]' value='' size='60'></div>";
    ...
    function showForm($ID, $options = array()) {
                ...
                foreach ($tt->predefined as $predeffield => $predefvalue) {
                    if (isset($default_values[$predeffield])) {
                        ...
                        if (((count($values['_predefined_fields']) == 0) && ($values[$predeffield] == $default_values[$predeffield])) || (isset($values['_predefined_fields'][$predeffield]) && ($values[$predeffield] == $values['_predefined_fields'][$predeffield])) || (isset($values['_tickettemplates_id']) && ($values['_tickettemplates_id'] != $tt->getID()))
                                ...
                                || (($predeffield == 'requesttypes_id') && empty($saved))) {
        ...
        if ($canupdate || $canupdate_descr) {
            ...
            Ajax::updateItemOnSelectEvent(array('dropdown_urgency' . $idurgency,
                'dropdown_impact' . $idimpact), $idajax, $CFG_GLPI["root_doc"] . "/ajax/priority.php", $params);
        }
        echo "</td>";
    ...
    static function showCentralCount($foruser = false) {
        ...
        if ($foruser) {
            ...
            if (Session::haveRight(self::$rightname, self::READGROUP) && isset($_SESSION["glpigroups"]) && count($_SESSION["glpigroups"])) {
    ...
    static function showListForItem(CommonDBTM $item) {
        ...
        if ($item->getID() && Ticket::isPossibleToAssignType($item->getType()) && self::canCreate()) {
            Html::showSimpleForm($CFG_GLPI["root_doc"] . "/front/ticket.form.php", '_add_fromitem', __('New ticket for this item...'), array('itemtype' => $item->getType(),
                'items_id' => $item->getID()));
        }
        echo "</div><div>";
    ...
    static function showVeryShort($ID, $forcetab = '') {
        ...
        if ($job->getFromDBwithData($ID, 0)) {
            ...
            if (isset($job->users[CommonITILActor::REQUESTER]) && count($job->users[CommonITILActor::REQUESTER])) {
                foreach ($job->users[CommonITILActor::REQUESTER] as $d) {
                    if ($d["users_id"] > 0) {
                        ...
                        $name = sprintf(__('%1$s %2$s'), $name, Html::showToolTip($userdata["comment"], array('link' => $userdata["link"],
                                    'display' => false)));
                        echo $name;
                    } else {
            ...
            $link = printf(__('%1$s %2$s'), $link, Html::showToolTip(nl2br(Html::Clean($content)), array('applyto' => 'ticket' . $job->fields["id"] . $rand,
                        'display' => false)));
            echo "</td>";
...
//                }
    ...
    function getTimelineItems() {
        ...
        $showpublic = Session::haveRightsOr("followup", array(TicketFollowup::SEEPUBLIC,
                    TicketFollowup::SEEPRIVATE)) || Session::haveRightsOr("task", array(TicketTask::SEEPUBLIC,
        ...
        if (!empty($this->fields['solution']) || !empty($this->fields['solutiontypes_id'])) {
            ...
            $timeline[$solution_date . "_solution"] = array('type' => 'Solution',
                'item' => array('id' => 0,
        ...
        if ((($this->fields['type'] == Ticket::DEMAND_TYPE) && (Session::haveRight('ticketvalidation', TicketValidation::VALIDATEREQUEST) || Session::haveRight('ticketvalidation', TicketValidation::CREATEREQUEST))) || (($this->fields['type'] == Ticket::INCIDENT_TYPE) && (Session::haveRight('ticketvalidation', TicketValidation::VALIDATEINCIDENT) || Session::haveRight('ticketvalidation', TicketValidation::CREATEINCIDENT)))) {
            ...
            foreach ($ticket_validations as $validations_id => $validation) {
                ...
                $timeline[$validation['submission_date'] . "_validation_" . $validations_id] = array('type' => 'TicketValidation',
                ...
                if (!empty($validation['validation_date'])) {
                    $timeline[$validation['validation_date'] . "_validation_" . $validations_id] = array('type' => 'TicketValidation',
                        'item' => array('id' => $validations_id,
                            'date' => $validation['validation_date'],
                            'content' => __('Validation request answer') . " : " .
                            _sx('status', ucfirst(TicketValidation::getStatus($validation['status'])))
    ...
    function showTimelineForm($rand) {
        ...
        //check global rights
        if (!Session::haveRight("ticket", Ticket::READMY) && !Session::haveRightsOr("followup", array(TicketFollowup::SEEPUBLIC,
     ...
     * */
    function referentielAdd(array $input, $options = array(), $history = true) {
        global $DB, $CFG_GLPI;
        ...
        if ($this->input && is_array($this->input)) {
            ...
            // Verifier si la partie 'assigné à' et 'observateur'
            // de ticket sont configurer à 'Group depuis un objet GLPI'
            ...
            $result_conf = $DB->query($select_conf);
            foreach ($result_conf as $row_conf) {
                if ($row_conf['actor_role'] == 'observer' && $row_conf['actor_type'] == 'glpi_object_group') {
                    $referentiel_validator = TRUE;
                }
                if ($row_conf['actor_role'] == 'assigned' && $row_conf['actor_type'] == 'glpi_object_group') {
                    $referentiel_assigned = TRUE;
                }
                if ($row_conf['actor_role'] == 'observer' && $row_conf['actor_type'] == 'group' && !empty($row_conf['actor_value'])) {
                    $specific_assigned_gv = $row_conf['actor_value'];
                }
                if ($row_conf['actor_role'] == 'assigned' && $row_conf['actor_type'] == 'group' && !empty($row_conf['actor_value']) && $row_conf['actor_value'] != "0") {
                    $specific_assigned_gr = $row_conf['actor_value'];
                }
            }
            if ($referentiel_validator == TRUE || $referentiel_assigned == TRUE) {
                $ref = new PluginFormcreatorReferentielsApplications();
                $group = $ref->referentielProcessAssignation();
                if ($referentiel_assigned == TRUE) {
                    $input['_groups_id_assign'] = $group['groupe_de_resolution'];
                } elseif (isset($specific_assigned_gr)) {
                    $input['_groups_id_assign'] = $specific_assigned_gr;
                }
                if ($referentiel_validator == TRUE || isset($specific_assigned_gv)) {
                    if (isset($group['groupe_de_valition'])) {
                        $groups_validation = $group['groupe_de_valition'];
                        $input['global_validation'] = 2;
                        $input['status'] = 4;
                        $input['_add_validation']['group'][0] = $group['groupe_de_valition'];
                    } elseif (isset($specific_assigned_gv)) {
                        $groups_validation = $specific_assigned_gv;
                        $input['global_validation'] = 2;
                        $input['status'] = 4;
                        $input['_add_validation']['group'][0] = $specific_assigned_gv;
                    }
                }
            } else {
                if (isset($specific_assigned_gv)) {
                    $groups_validation = $specific_assigned_gv;
                    $input['global_validation'] = 2;
                    $input['status'] = 4;
                    $input['_add_validation']['group'][0] = $specific_assigned_gv;
                }
                if (isset($specific_assigned_gr)) {
                    $input['_groups_id_assign'] = $specific_assigned_gr;
                }
            }
            // assigner le SLA et la catégorie selon le référentiel
            // assigner la catégorie
            $select_categorie = 'SELECT `id` FROM `glpi_itilcategories` WHERE `completename` LIKE \'' . $_SESSION['referentiel_actif']['categorie'] . '\';';
            $result_categorie = $GLOBALS['DB']->query($select_categorie);
            if ($result_categorie->num_rows > 0) {
                $row_categorie = $GLOBALS['DB']->fetch_array($result_categorie);
                $input['itilcategories_id'] = $row_categorie['id'];
            }

            //assigner le SLT
            $select_slts = 'SELECT `id` FROM `glpi_slts` WHERE `name` LIKE \'' . $_SESSION['referentiel_actif']['slas'] . '\';';
            $result_slts = $GLOBALS['DB']->query($select_slts);
            if ($result_slts->num_rows > 0) {
                $row_slts = $GLOBALS['DB']->fetch_array($result_slts);
                $input['slts_ttr_id'] = $row_slts['id'];
            }

            $this->input = $input;
            //soumettre au moteur de règles
        ...
        if ($this->input && is_array($this->input)) {
            $this->fields = array();
            $_SESSION['glpi_table_of']['PluginFormcreatorExtensionTicket'] = 'glpi_tickets';
            ...
            if ($this->checkUnicity(true, $options)) {
                if ($this->addToDB()) {
                    ...
                    // ajout de groupe de validation
                    if (!empty($groups_validation)) {
                        $ticket = $this->fields;
                        $ref = new PluginFormcreatorReferentielsApplications();
                        $groups = $ref->addGroupsValidation($groups_validation, $ticket);
                    }
