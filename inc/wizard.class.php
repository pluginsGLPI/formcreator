...
class PluginFormcreatorWizard {
   ...
   public static function header($title) {
      ...
      $link = new Link;
      $external_links_prefix = PluginFormcreatorEntityconfig::getUsedConfig('external_links_prefix', $_SESSION['glpiactive_entity'], PluginFormcreatorEntityconfig::CONFIG_PARENT_STRING);
      $ar_links = $link->find("name LIKE '". $external_links_prefix ."%'");
      foreach ($ar_links as $id => $a_link) {
          ...
          */
         $label = trim(str_replace($external_links_prefix, '', $a_link['name']));
          ...
          */
         $external_links_icon = PluginFormcreatorEntityconfig::getUsedConfig('external_links_icon', $_SESSION['glpiactive_entity'], PluginFormcreatorEntityconfig::CONFIG_PARENT_STRING);
         preg_match_all("/^". $external_links_icon ."(.*)$/m", $a_link['data'], $found);
         $icon = "fa fa-globe";
          ...
          */
         $external_links_title = PluginFormcreatorEntityconfig::getUsedConfig('external_links_title', $_SESSION['glpiactive_entity'], PluginFormcreatorEntityconfig::CONFIG_PARENT_STRING);
         preg_match_all("/^". $PFC_CONFIG['external_links_title'] ."(.*)$/m", $a_link['data'], $found);
         $title = $a_link['name'];
         if ((count($found) > 0) and (count($found[0]) > 0)) {
            $title = trim(str_replace($external_links_title, '', $found[0][0]));
         }
         /*
